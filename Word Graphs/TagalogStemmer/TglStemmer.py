...
def clean_stemmed(token, CLEANERS, REPITITION):
	...
	if len(token) > 0 and not check_vowel(token[0]) and not check_consonant(token[0]):
...
def check_validation(token):
	with open('./TagalogStemmer/validation.txt', 'r', encoding='utf-8', errors='ignore') as valid:
